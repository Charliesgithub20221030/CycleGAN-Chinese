...
class cycle_gan():
    ...
    def __init__(self, args, sess):
        ...
        # trivial things
        self.lstm_length = [self.sequence_length +
    ...
    def build_model(self):
        ...
        def get_all_variables():
            self.generator_X2Y_variables = [v for v in tf.trainable_variables(
            ) if v.name.startswith("generator_X2Y") or v.name.startswith("generator_word")]
            for v in self.generator_X2Y_variables:
                print(v.name)
            self.generator_Y2X_variables = [v for v in tf.trainable_variables(
            ) if v.name.startswith("generator_Y2X") or v.name.startswith("generator_word")]
            ...
            self.discriminator_X_variables = [v for v in tf.trainable_variables(
            ) if v.name.startswith("discriminator_X") or v.name.startswith("discriminator_word")]
            for v in self.discriminator_X_variables:
                print(v.name)
            self.discriminator_Y_variables = [v for v in tf.trainable_variables(
            ) if v.name.startswith("discriminator_Y") or v.name.startswith("discriminator_word")]
            ...
            self.generator_saver = tf.train.Saver(
                self.generator_variables, max_to_keep=6)
            self.discriminator_saver = tf.train.Saver(
                self.discriminator_variables, max_to_keep=2)
        ...
        def get_discriminator_loss(real_sample_score, false_sample_score, gradient_penalty):
            ...
            discriminator_loss = -(real_sample_score -
        ...
        def get_L2_loss(targets, outputs):
            ...
            loss = tf.reduce_sum(tf.pow(targets - outputs, 2)) / \
        ...
        def get_gradient_penalty(generator_outputs_embedded, real_sample_embedded, dis_fn):
            ...
            slopes = tf.sqrt(tf.reduce_sum(
                tf.square(gradients), reduction_indices=[1, 2]))
        ...
        def build_XYX_graph():
            with tf.variable_scope("XYX_inputs"):
                self.X2Y_inputs = tf.placeholder(
                    dtype=tf.int32, shape=(self.batch_size, self.sequence_length))
                X2Y_inputs = tf.concat([self.X2Y_inputs, EOS_slice], axis=1)
                X2Y_inputs = tf.nn.embedding_lookup(
                    word_embedding_matrix, X2Y_inputs)

                self.real_Y_sample = tf.placeholder(
                    dtype=tf.int32, shape=(self.batch_size, self.sequence_length))
                real_Y_sample = tf.concat(
                    [self.real_Y_sample, EOS_slice], axis=1)
                real_Y_sample = tf.nn.embedding_lookup(
                    word_embedding_matrix, real_Y_sample)

                Y2X_decoder_inputs = tf.concat(
                    [BOS_slice, self.X2Y_inputs], axis=1)
                Y2X_decoder_inputs = tf.nn.embedding_lookup(
                    word_embedding_matrix, Y2X_decoder_inputs)

                X2Y_decoder_inputs = tf.zeros(
                    [self.batch_size, self.sequence_length+1], dtype=tf.int32) + self.BOS
                if self.mode == 'pretrain':
                    X2Y_decoder_inputs = tf.concat(
                        [BOS_slice, self.X2Y_inputs], axis=1)
                X2Y_decoder_inputs = tf.nn.embedding_lookup(
                    word_embedding_matrix, X2Y_decoder_inputs)
            ...
            with tf.variable_scope("discriminator_Y") as scope:
                ...
                self.false_Y_sample_score = tf.reduce_mean(
                    false_Y_sample_score)

                scope.reuse_variables()

                real_Y_sample_score = discriminator(real_Y_sample)
                dis_Y_penalty = get_gradient_penalty(
                    X2Y_outputs, real_Y_sample, discriminator)
                self.gpy = dis_Y_penalty

            with tf.variable_scope("XYX_loss") as scope:

                self.X2Y_reconstruction_loss = get_L2_loss(
                    X2Y_inputs, Y2X_rec_outputs)

                self.discriminator_Y_loss = get_discriminator_loss(
                    real_Y_sample_score, false_Y_sample_score, dis_Y_penalty)
        ...
        def build_YXY_graph():
            with tf.variable_scope("YXY_inputs") as scope:
                self.Y2X_inputs = tf.placeholder(
                    dtype=tf.int32, shape=(self.batch_size, self.sequence_length))
                Y2X_inputs = tf.concat([self.Y2X_inputs, EOS_slice], axis=1)
                Y2X_inputs = tf.nn.embedding_lookup(
                    word_embedding_matrix, Y2X_inputs)

                self.real_X_sample = tf.placeholder(
                    dtype=tf.int32, shape=(self.batch_size, self.sequence_length))
                real_X_sample = tf.concat(
                    [self.real_X_sample, EOS_slice], axis=1)
                real_X_sample = tf.nn.embedding_lookup(
                    word_embedding_matrix, real_X_sample)

                X2Y_decoder_inputs = tf.concat(
                    [BOS_slice, self.Y2X_inputs], axis=1)
                X2Y_decoder_inputs = tf.nn.embedding_lookup(
                    word_embedding_matrix, X2Y_decoder_inputs)

                Y2X_decoder_inputs = tf.zeros(
                    [self.batch_size, self.sequence_length+1], dtype=tf.int32) + self.BOS
                if self.mode == 'pretrain':
                    Y2X_decoder_inputs = tf.concat(
                        [BOS_slice, self.Y2X_inputs], axis=1)
            ...
            with tf.variable_scope("discriminator_X") as scope:
                ...
                self.false_X_sample_score = tf.reduce_mean(
                    false_X_sample_score)

                scope.reuse_variables()

                real_X_sample_score = discriminator_X(real_X_sample)
                dis_X_penalty = get_gradient_penalty(
                    Y2X_outputs, real_X_sample, discriminator_X)

            with tf.variable_scope("YXY_loss") as scope:

                self.Y2X_reconstruction_loss = get_L2_loss(
                    Y2X_inputs, X2Y_rec_outputs)

                self.discriminator_X_loss = get_discriminator_loss(
                    real_X_sample_score, false_X_sample_score, dis_X_penalty)
        ...
        with tf.variable_scope('generator_loss') as scope:
            self.generator_X2Y_loss = (
                self.Y2X_reconstruction_loss + self.X2Y_reconstruction_loss)*2.0 - self.false_Y_sample_score
            self.generator_Y2X_loss = (
                self.Y2X_reconstruction_loss + self.X2Y_reconstruction_loss)*2.0 - self.false_X_sample_score
    ...
    def pretrain(self):
        ...
        for X_batch, Y_batch in self.utils.pretrain_generator_data_generator():
            ...
            _, _, loss_X2Y, loss_Y2X, tt = self.sess.run(
    ...
    def train(self):
        ...
        self.generator_saver.restore(
            self.sess, tf.train.latest_checkpoint(gen_model_dir))
        if self.load_model:
            print('load model from:', self.model_dir)
            self.discriminator_saver.restore(
                self.sess, tf.train.latest_checkpoint(dis_model_dir))
        ...
        for real_X_batches, real_Y_batches in self.utils.gan_data_generator():
            ...
            if step % saving_step == 0:
                print('saving model!!!!......')
                self.discriminator_saver.save(
                    self.sess, dis_model_path, global_step=step)
