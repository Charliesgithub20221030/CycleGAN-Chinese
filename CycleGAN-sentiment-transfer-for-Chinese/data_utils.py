...
class data_utils():
    def __init__(self, args):
        # Data source
        self.data_path = args.data_source
        self.num_batch = 12
    ...
    def sent2id(self, sent):
        ...
        i = 0
        # print(sent_list) # check segment of sentence, English ok, Chinese not
    ...
    def gan_data_generator(self):
        ...
        for one_X, one_Y in zip(self.X_data_generator(), self.Y_data_generator()):
            ...
            if len(one_X_batch) == self.batch_size*self.num_batch:
                one_X_batch = np.array(one_X_batch).reshape(
                    self.num_batch, self.batch_size, -1)
                one_Y_batch = np.array(one_Y_batch).reshape(
                    self.num_batch, self.batch_size, -1)
